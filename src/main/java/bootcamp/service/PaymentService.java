...
public class PaymentService {
    ...
    public Boolean makePayment(int supplier, double total, int invoiceId){
        ...
        switch(supplier){
            case 0:
                builder = UriComponentsBuilder.fromUriString("http://" + supplier_a_url + "/payment").port(8000);
                complete = restTemplate.postForObject(builder.toUriString(), payment, Boolean.class);
                break;
            case 1:
                builder = UriComponentsBuilder.fromUriString("http://" + supplier_b_url + "/payment").port(8000);
                complete = restTemplate.postForObject(builder.toUriString(), payment, Boolean.class);
                break;
            case 2:
                builder = UriComponentsBuilder.fromUriString("http://" + supplier_c_url + "/payment").port(8000);
                complete = restTemplate.postForObject(builder.toUriString(), payment, Boolean.class);
                break;
            default:
                break;
